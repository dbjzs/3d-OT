...
import torch
import numpy as np
from lib_3d_OT.ottools.lattice import Lattice
project_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if project_dir not in sys.path:
    sys.path.append(project_dir)


from lib_3d_OT.ottools.utils import iterate_in_chunks
...
def compute_loss_unsupervised(recon_flow, corr_conf, target_pc_recon, graph, pclouds, args,device):
    ...
    return loss, target_recon_loss, smooth_flow_loss, div_flow_loss
...
def find_nearest_K_neighbors(pointcloud, k):
    ...
    dist = torch.cdist(pointcloud, pointcloud, p=2) 
    _, indx = torch.topk(dist, k=k+1, largest=False, dim=-1)
    return indx[:, 1:]
