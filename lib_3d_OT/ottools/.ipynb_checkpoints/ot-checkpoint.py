...
def sinkhorn(feature1, feature2, pcloud1, pcloud2, epsilon, gamma, max_iter, sim_k=50, dist_k=10):
    ...
    for b in range(batch_size):
        for n in range(N):
            distances = torch.norm(pcloud1[b, n, :].unsqueeze(0) - pcloud2[b, :, :], p=2, dim=-1)

            _, dist_indices = torch.topk(distances, k=dist_k, largest=False)
            
            dist_candidate_indices = dist_indices
            candidate_sim_values = S[b, n, dist_candidate_indices]
            _, sim_indices_from_dist = torch.topk(candidate_sim_values, k=sim_k, largest=True)
