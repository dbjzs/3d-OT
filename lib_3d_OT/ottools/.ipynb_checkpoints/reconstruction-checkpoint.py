...
def get_support_matrix(pcloud1, pcloud2, scale_factor=2.0):
    """
    Compute global adaptive support matrix for two point clouds.
    The distance threshold is based on the global average distance between points.

    Parameters
    ----------
    pcloud1 : torch.Tensor
        Point cloud 1. Size B x N x 2.
    pcloud2 : torch.Tensor
        Point cloud 2. Size B x M x 2.
    scale_factor : float
        Scale factor to expand the global threshold.
    ...
    pairwise_dist1 = torch.cdist(pcloud1, pcloud1, p=2)  # B x N x N
    pairwise_dist2 = torch.cdist(pcloud2, pcloud2, p=2)  # B x M x M

    # Compute the mean distance as the global threshold
    global_thresh1 = pairwise_dist1.mean(dim=(1, 2), keepdim=True)  # B x 1
    global_thresh2 = pairwise_dist2.mean(dim=(1, 2), keepdim=True)  # B x 1
    global_thresh = torch.max(global_thresh1, global_thresh2) * scale_factor  # Scale up threshold

    # Compute squared pairwise distances between the two point clouds
    ...
    # Apply global adaptive threshold to define support matrix
