...
import torch
import numpy as np
from tools.lattice import Lattice
...
def compute_loss_unsupervised(recon_flow, corr_conf, target_pc_recon, graph, pclouds, args,device):
    """
    ...
    target_pc_input = pclouds[1]
    src_pc_input = pclouds[0]
    ...
    if args['use_div_flow'] and args['div_flow_loss_weight'] > 0:
        ...
        criterion=Lattice(nb=args['div_neighbor'],spacing=args['lattice_steps'],batchsize=recon_flow.shape[0],device=device)
...
def chamfer_dist_2d_pt(pc1, pc2, backward_dist_weight=0.0, chunk_size=2048):
    b = pc1.shape[0]
    n = pc1.shape[1]
    m = pc2.shape[1]
    device = pc1.device
...
def chamfer_loss(pc1, pc2, point_weight=None, backward_dist_weight=1.0, mask=None, use_chamfer_cuda=True):
    ...
    dist1, dist2, idx1, idx2 = chamfer_dist_2d_pt(pc1, pc2, backward_dist_weight)
...
def find_nearest_K_neighbors(pointcloud, k):
    """
    Find the K nearest neighbors for each point in the point cloud using PyTorch.
    
    Parameters:
    - pointcloud: torch.Tensor of shape [N, C] (N points, C dimensions).
    - k: int, number of nearest neighbors to find.
    
    Returns:
    - indx: torch.Tensor of shape [N, K], indices of the K nearest neighbors for each point.
    """
    # Compute pairwise distances
    dist = torch.cdist(pointcloud, pointcloud, p=2)  # [N, N], Euclidean distance
    # Sort distances to get nearest neighbors
    _, indx = torch.topk(dist, k=k+1, largest=False, dim=-1)  # Get top K+1 (exclude self)
    return indx[:, 1:]
