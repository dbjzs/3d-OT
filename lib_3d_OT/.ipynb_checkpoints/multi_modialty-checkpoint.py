...
class SetTransformerEncoder(nn.Module):
    def __init__(self, input_dim,n_heads=4, num_transfomer_layers=3,hidden=32):
        ...
        self.transformer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=8*hidden, nhead=n_heads),
            num_layers=num_transfomer_layers
        )
...
class extractModel(nn.Module):
    def __init__(self, input_dim,hidden_dim=32, n_heads=4, n_layers=3):
        ...
        self.weight1 = nn.Parameter(torch.tensor(1.0))  # 默认权重为 1.0
        self.weight2 = nn.Parameter(torch.tensor(1.0))
        

        # 解码器（共享结构，独立权重）
        self.decoder1 = nn.Sequential(
            nn.Linear(8 * hidden_dim, 4 * hidden_dim),  # Reduce dimensionality
            nn.ReLU(),
            nn.Linear(4 * hidden_dim, 2 * hidden_dim),  # Further reduce
            nn.ReLU(),
            nn.Linear(2* hidden_dim, input_dim),  # Output 3000-dimensional features
        )
    ...
    def forward(self, graph1, graph2):
        ...
        return recon1, recon2, mixed_modal_features  # 返回重构结果和注意力权重
...
class UnifiedModel(nn.Module):
    ...
    def get_encoder_results(self,model1,model2,pcloud_list1,pcloud_list2):
        ...
        # 如果你需要返回 Tensor 类型，可以重新转换为 Tensor
        return recon1,recon4,pcloud_list1[0],pcloud_list2[0]
...
def train(model, pcloud_list, optimizer, scheduler, device, nb_epochs=100, use_corr_conf=True, use_smooth_flow=True, use_div_flow=True, args=None):
    """
    Unified training function for GNN and flow reconstruction over all time points together.
    """
    model = model.to(device)

    for epoch in range(nb_epochs):  # 所有时间点共享一个训练循环
        model.train()
        optimizer.zero_grad()
        total_loss = 0.0
        for idx in range(len(pcloud_list) - 1):  # 遍历所有时间点对

            graph1, graph2 = pcloud_list[idx], pcloud_list[idx + 1]

            recon_flow, corr_conf, target_recon, graph,pclouds = model(graph1, graph2)

            loss, target_recon_loss, corr_conf_loss, smooth_flow_loss, div_flow_loss = compute_loss_unsupervised(
            ...
            # 梯度更新
        #optimizer.zero_grad()
        total_loss.backward()  # 对每个时间对的损失进行反向传播
        optimizer.step()
        scheduler.step()  # 每个 batch 更新学习率

        print(f"\rTime Pair {idx}, Epoch {epoch}/{nb_epochs}, total_loss: {total_loss.item():.4f},smooth_flow_loss: {smooth_flow_loss.item():.4f} "
